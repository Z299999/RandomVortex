...
# ============================
nu = 0.01          # Viscosity (here zero so no noise)
...
# For torus simulation, we take the fundamental domain D as:
...
# ---------------------------
# Grid Generator for Torus Simulation
# ---------------------------
def generate_simulation_grid():
    """
    Generates five grids for torus simulation:
...
def simulate_vortex_trajectories():
    ...
    for step in range(num_steps):
        ...
        # Copy/update D₃ from D (shift down by 4):
        for i in range(traj_D3[0].shape[0]):
            for j in range(traj_D3[0].shape[1]):
                x, y = grid_D3[i, j]
                i_dense = int(round((x - (-2)) / hd))
                j_dense = int(round((y + 4 - 0) / hd))
                if i_dense < traj_D[step+1].shape[0] and j_dense < traj_D[step+1].shape[1]:
                    traj_D3[step+1, i, j] = traj_D[step+1, i_dense, j_dense] + np.array([0, -4])
        # Copy/update D₄ from D (shift up by 4):
        for i in range(traj_D4[0].shape[0]):
            for j in range(traj_D4[0].shape[1]):
                ...
                j_dense = int(round((y - 4 - 0) / hd))
                if i_dense < traj_D[step+1].shape[0] and j_dense < traj_D[step+1].shape[1]:
                    traj_D4[step+1, i, j] = traj_D[step+1, i_dense, j_dense] + np.array([0, 4])
...
# ---------------------------
# Boat Simulation on the Torus (for region D with periodic boundary conditions)
# ---------------------------
...
# ---------------------------
# Torus Mapping Functions
# ---------------------------
def to_torus(point):
    """
    Maps a point (x,y) in [-2,2)×[0,4) to a torus in ℝ³.
    We set u = (x+2)*(π/2) and v = y*(π/2), then:
    ...
    R = 3
    r = 1
    X = (R + r*np.cos(v_angle)) * np.cos(u_angle)
    Y = (R + r*np.cos(v_angle)) * np.sin(u_angle)
    Z = r*np.sin(v_angle)
    return np.array([X, Y, Z])

def velocity_to_torus(point, vel):
    """
    Transforms a 2D velocity vector at 'point' from rectangular coordinates
    to the corresponding 3D velocity on the torus using the Jacobian.
    """
    x, y = point
    u_vel, v_vel = vel
    u_angle = (x + 2) * (np.pi/2)
    ...
    R = 3
    r = 1
    dFdx = (np.pi/2)*np.array([- (R + r*np.cos(v_angle))*np.sin(u_angle),
                                (R + r*np.cos(v_angle))*np.cos(u_angle),
                                0])
    dFdy = (np.pi/2)*np.array([-r*np.sin(v_angle)*np.cos(u_angle),
                                -r*np.sin(v_angle)*np.sin(u_angle),
                                r*np.cos(v_angle)])
    return u_vel*dFdx + v_vel*dFdy

# ---------------------------
# Prepare the Static Torus Surface for 3D Plotting
# ---------------------------
...
def init_3d(ax):
    ax.plot_surface(X_torus, Y_torus, Z_torus, alpha=0.2, rstride=2, cstride=2, edgecolor='none')
    ax.set_xlim(-5, 5)
    ax.set_ylim(-5, 5)
    ax.set_zlim(-1.5, 1.5)
    ax.set_xlabel('X')
...
def update(frame):
    ...
    # Transform velocity query points and vectors into torus coordinates.
    torus_positions = np.array([to_torus(pt) for pt in query_all])
    torus_velocities = np.array([velocity_to_torus(pt, (u, v)) 
                                 for pt, u, v in zip(query_all, U_all, V_all)])
    Xq = torus_positions[:, 0]
    Yq = torus_positions[:, 1]
    Zq = torus_positions[:, 2]
    Uq = torus_velocities[:, 0]
    Vq = torus_velocities[:, 1]
    Wq = torus_velocities[:, 2]
    
    ax3d.quiver(Xq, Yq, Zq, Uq, Vq, Wq, length=0.5, normalize=True,
                color='black', pivot='middle')
    
    # Transform boat positions into torus coordinates.
    boats_rect = boat_positions[frame]
    boats_torus = np.array([to_torus(pt) for pt in boats_rect])
    ax3d.scatter(boats_torus[:, 0], boats_torus[:, 1], boats_torus[:, 2],
                 s=20, c=boat_colors, depthshade=True)
    
    ax3d.set_title(f"3D Torus View (t={t_current:.2f})")
    return
...
save_path = os.path.join("animation", "2d_3d_torus.mp4")
