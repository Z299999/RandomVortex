...
# ---------------------------
# Copy Initial Vorticity (Klein Bottle modifications)
# ---------------------------
def copy_initial_vorticity(grid_D, w_D, grid_D1, w_D1, grid_D2, w_D2, grid_D3, w_D3, grid_D4, w_D4):
    ...
    # D₃: twisted: (x,y) -> (-x, y+4) with opposite sign.
...
# ---------------------------
# Boat Simulation with Twisted Boundaries
# ---------------------------
...
# ---------------------------
# Velocity Query Grids for Visualization
# ---------------------------
...
# ---------------------------
# Precompute Global Velocity Scale using t=0
# ---------------------------
u0 = uFuncs[0]
vels = np.array([np.linalg.norm(u0(pt)) for pt in query_all_initial])
global_vmin = vels.min()
global_vmax = vels.max()

# ---------------------------
# Create the Animation with 2 Subplots
# ---------------------------
...
def init_3d(ax):
    surf = ax.plot_surface(X_klein, Y_klein, Z_klein, alpha=0.2, rstride=2, cstride=2,
                           edgecolor='none')
    ax.set_xlim(-5, 5)
    ax.set_ylim(-5, 5)
    ax.set_zlim(-3, 3)
...
def update(frame):
    ...
    # --- Background: Color layer (fixed scale) ---
    x_bg = np.arange(window_x[0], window_x[1] + hd, hd)
    y_bg = np.arange(window_y[0], window_y[1] + hd, hd)
    X_bg, Y_bg = np.meshgrid(x_bg, y_bg)
    vel_bg = np.zeros_like(X_bg)
    for i in range(X_bg.shape[0]):
        for j in range(X_bg.shape[1]):
            vel_bg[i, j] = np.linalg.norm(u_func(np.array([X_bg[i, j], Y_bg[i, j]])))
    cmesh = ax2d.pcolormesh(X_bg, Y_bg, vel_bg, shading='auto', cmap='viridis',
    ...
    # ----- 3D Update -----
    ax3d.cla()
    # --- Background: Colored Klein bottle surface ---
    # Map (u,v) from U_mesh,V_mesh to (x,y) in D domain:
    x_surf = (2/np.pi) * U_mesh - 2
    y_surf = (2/np.pi) * V_mesh
    vel_surf = np.zeros_like(U_mesh)
    for i in range(U_mesh.shape[0]):
        for j in range(U_mesh.shape[1]):
            vel_surf[i, j] = np.linalg.norm(u_func(np.array([x_surf[i, j], y_surf[i, j]])))
    norm = plt.Normalize(vmin=global_vmin, vmax=global_vmax)
    facecolors = cm.viridis(norm(vel_surf))
    ax3d.plot_surface(X_klein, Y_klein, Z_klein, facecolors=facecolors, rstride=1, cstride=1,
                      shade=False, alpha=0.8, edgecolor='none')
    
    # --- Velocity Field Arrows in 3D ---
