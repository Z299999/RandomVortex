...
# Define the discrete vorticity: Set w0 = 1/(h^2) at each active vortex.
...
# Note: for active vortices, w0[z]*h^2 = 1.
    ...
    for i in range(num_particles):
        for rho in range(N):
            ...
            for z in active_indices:
                ...
                # Multiply by w0[z]*h^2, which is 1 for our active vortices.
                drift += temp / N
...
# ----------------------------------------------------------------
# We will compute u(x, t) = sum_{z in active_indices} (1/N)*sum_{sigma} K_delta( X_z^(sigma)(t) - x )
    ...
    for i in range(xx.shape[0]):
        for j in range(xx.shape[1]):
            ...
            for z in active_indices:
                ...
                temp += inner / N
            vel = temp  # Each active vortex contributes a strength of 1.
            U[i, j] = vel[0]
            V[i, j] = vel[1]
...
for i in range(num_particles):
    ...
    for rho in range(N):
        plt.plot(trajectories[:, i, rho, 0], trajectories[:, i, rho, 1],
...
for idx, vortex in enumerate(active_indices):
    for rho in range(N):
        plt.plot(trajectories[:, vortex, rho, 0], trajectories[:, vortex, rho, 1],
...
save_path = os.path.join("animation", "velocity_field_same.mp4")
