...
# h1: partition size in both x and y directions for the fine mesh
h0 = 0.7    # Coarse mesh grid spacing (both x and y)
h1 = 0.2    # Fine mesh grid spacing in both x and y directions

layer_thickness = 0.6 # used as the dividing y-level between fine and coarse grids
...
def generate_nonuniform_grid_D():
    ...
    and a finer grid near the boundary (x=0 and y=0). The coarse grid uses a uniform partition
    of size h0 in both x and y directions, while the fine grid uses h1 for the x-axis and
    h1 for the y-axis.
    
    The fine grid covers y in [0, hh] and x in [0, hh], while the coarse grid covers the rest.
    
    Returns:
      A dictionary with keys 'coarse' and 'fine'. Each value is a tuple (grid, A)
      where grid is a list of points [x, y] and A is the corresponding area array.
    """
    x1, x2 = region_x
    y1, y2 = region_y
    hh = layer_thickness  # dividing line between fine and coarse grid in both x and y directions

    # Coarse grid: x in [hh, x2] and y in [hh, y2] with spacing h0 in both x and y
    num_x_coarse = int((x2 - hh) / h0) + 1
    num_y_coarse = int((y2 - hh) / h0) + 1
    x_coarse = np.linspace(hh, x2, num_x_coarse)
    y_coarse = np.linspace(hh, y2, num_y_coarse)
    xx_coarse, yy_coarse = np.meshgrid(x_coarse, y_coarse, indexing='ij')
    grid_coarse = np.stack((xx_coarse, yy_coarse), axis=-1).reshape(-1, 2).tolist()
    A_coarse = h0 * h0 * np.ones(len(grid_coarse))
    
    # Fine grid: y in [0, hh] and x in [0, x2] union x in [0, hh] and y in [hh, y2] with spacing h1
    num_x_fine = int((x2 - x1) / h1) + 1
    num_y_fine = int(hh / h1) + 1
    x_fine = np.linspace(x1, x2, num_x_fine)
    y_fine = np.linspace(y1, hh, num_y_fine, endpoint=False)
    xx_fine, yy_fine = np.meshgrid(x_fine, y_fine, indexing='ij')
    grid_fine_1 = np.stack((xx_fine, yy_fine), axis=-1).reshape(-1, 2)
    
    num_x_fine_2 = int(hh / h1) + 1
    num_y_fine_2 = int((y2 - hh) / h1) + 1
    x_fine_2 = np.linspace(x1, hh, num_x_fine_2, endpoint=False)
    y_fine_2 = np.linspace(hh, y2, num_y_fine_2)
    xx_fine_2, yy_fine_2 = np.meshgrid(x_fine_2, y_fine_2, indexing='ij')
    grid_fine_2 = np.stack((xx_fine_2, yy_fine_2), axis=-1).reshape(-1, 2)
    
    grid_fine = np.concatenate((grid_fine_1, grid_fine_2), axis=0).tolist()
    A_fine = h1 * h1 * np.ones(len(grid_fine))
    
    print(f"Coarse grid size: {len(grid_coarse)} points")
    print(f"Fine grid size: {len(grid_fine)} points")
...
def plot_grid(grid_coarse, grid_fine):
    """
    Plots the coarse and fine grids to visualize their shapes.
    """
    ...
    coarse_points = np.array(grid_coarse)
    ax.scatter(coarse_points[:, 0], coarse_points[:, 1], color='red', label='Coarse Grid', s=10)

    # Plot fine grid points
    fine_points = np.array(grid_fine)
    ax.scatter(fine_points[:, 0], fine_points[:, 1], color='blue', label='Fine Grid', s=10)

    ax.legend()
    plt.show()

# Print out mesh grid sizes
print(f"Coarse grid size: {len(grid_coarse)} points")
print(f"Fine grid size: {len(grid_fine)} points")

# Plot the grids for debugging
plot_grid(grid_coarse, grid_fine)
    ...
    def u_func(x):
        ...
        for i in range(n1):
            for j in range(n2):
                ...
                contrib1 = indicator_D(pos) * K_Q(pos, x, delta)
                contrib2 = indicator_D(reflect(pos)) * K_Q(reflect(pos), x, delta)
