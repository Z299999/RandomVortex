...
h1 = 0.3    # Fine mesh grid spacing in both x and y directions

layer_thickness = 0.5 # used as the dividing y-level between fine and coarse grids
...
def generate_nonuniform_grid_D():
    ...
    # Fine grid: y in [y1, y3] with spacing h1 (x) and h1 (y)
    num_x_fine = int((x2 - x1) / h1) + 1
    num_y_fine = int((y3 - y1) / h1) + 1
    ...
    A_fine = h1 * h1 * np.ones((num_x_fine, num_y_fine))
...
save_path = os.path.join("animation", "h0h1.mp4")
