...
h2 = h1 / (h1 // h2_0)         # h2 is chosen from h1 and h2_0
...
def generate_nonuniform_grid_D():
    """
    Generates a nonuniform grid: a coarse grid over most of the domain and a
    finer grid near the boundary (y=0).
    """
    x1, x2 = region_x
    y1, y2 = region_y
    y3 = layer_thickness
    ...
    A_fine = h2 * h2 * np.ones((num_x_fine, num_y_fine))
...
def simulate_vortex_trajectories():
    ...
    Pre-computes two initial steps (l = 1 and l = 2) with the velocity field at t=0.
    """
    ...
    # Pre-compute initial step l=1 using velocity field at t=0:
...
# Boat Simulation
# ---------------------------
def generate_nonuniform_query_grid():
    """
    Generates the original nonuniform grid (coarse+fine) to be used for velocity field
    computation and animation.
    """
    grids = generate_nonuniform_grid_D()
    grid_coarse, _ = grids['coarse']
    grid_fine, _ = grids['fine']
    query_grid = np.concatenate((grid_coarse.reshape(-1, 2), grid_fine.reshape(-1, 2)), axis=0)
    return query_grid

def generate_uniform_boat_grid():
    """
    Generates a uniform grid for boat simulation over the entire domain
    with grid spacing d_boat = h2.
    """
    x1, x2 = region_x
    y1, y2 = region_y
    d_boat = h1  # grid size for boat simulation
    num_x = int((x2 - x1) / d_boat) + 1
    num_y = int((y2 - y1) / d_boat) + 1
    x_vals = np.linspace(x1, x2, num_x)
    y_vals = np.linspace(y1, y2, num_y)
    xx, yy = np.meshgrid(x_vals, y_vals, indexing='ij')
    boat_grid = np.stack((xx, yy), axis=-1).reshape(-1, 2)
    return boat_grid

def simulate_boats(uFuncs):
    """
    Simulates boat trajectories on a uniform grid. Boats are advected solely
    by the drift (using uFuncs) with no noise.
    """
    boat_grid = generate_uniform_boat_grid()  # use the uniform boat grid
...
# For velocity field display we use the original nonuniform query grid.
query_grid = generate_nonuniform_query_grid()

def compute_velocity_field(u_func, query_points):
    """
    Computes the velocity field at query points using the provided u_func.
    """
...
# Plot boat positions as a scatter plot with smaller blue dots.
boat_scatter = ax.scatter(boat_positions[0, :, 0], boat_positions[0, :, 1],
                          s=5, color='blue', zorder=3)
